...
export function validateStartupEnvironment() {
  ...
  if (appEnv === "production") {
    ...
    if (!["pinata", "arweave", "irys"].includes(process.env.FINAL_ASSET_STORAGE_PROVIDER ?? process.env.ASSET_STORAGE_PROVIDER ?? "mock")) issues.push({ code: "MUTABLE_FINAL_STORAGE", severity: "fatal", message: "FINAL_ASSET_STORAGE_PROVIDER must be immutable storage: pinata, arweave, or irys." });
    ...
    if ((process.env.FINAL_PRODUCTION_ASSETS_APPROVED ?? "false") === "true" || (process.env.REQUIRED_LAUNCH_ASSET_STATUS ?? "") === "FINAL_PRODUCTION") {
      if (!(process.env.APPROVED_TRAIT_MANIFEST_URI || process.env.APPROVED_TRAIT_MANIFEST_HASH)) issues.push({ code: "APPROVED_TRAIT_MANIFEST_MISSING", severity: "fatal", message: "FINAL_PRODUCTION requires APPROVED_TRAIT_MANIFEST_URI or APPROVED_TRAIT_MANIFEST_HASH." });
      if (!process.env.FINAL_ASSET_PROVENANCE_HASH) issues.push({ code: "FINAL_PROVENANCE_MISSING", severity: "fatal", message: "FINAL_PRODUCTION requires FINAL_ASSET_PROVENANCE_HASH." });
      if (!process.env.FINAL_RENDERER_VERSION) issues.push({ code: "FINAL_RENDERER_VERSION_MISSING", severity: "fatal", message: "FINAL_PRODUCTION requires FINAL_RENDERER_VERSION." });
      if (["ai", "openai", "imagen", "gemini", "stability", "flux", "bfl"].includes((process.env.DESIGN_MODEL_PROVIDER ?? "").toLowerCase())) issues.push({ code: "AI_FINAL_DESIGN_PROVIDER", severity: "fatal", message: "AI providers cannot be used for final 10k generation." });
      if (["ai", "openai", "imagen", "gemini", "stability", "flux", "bfl"].includes((process.env.LAYER_PACK_PROVIDER ?? "").toLowerCase())) issues.push({ code: "AI_FINAL_LAYER_PROVIDER", severity: "fatal", message: "AI providers cannot be used for final layer-pack generation." });
      if (["ai", "openai", "imagen", "gemini", "stability", "flux", "bfl"].includes((process.env.LEGENDARY_ASSET_PROVIDER ?? "").toLowerCase())) issues.push({ code: "AI_FINAL_LEGENDARY_PROVIDER", severity: "fatal", message: "AI providers cannot be used for final legendary asset generation." });
    }
    ...
    if (!process.env.COMMUNITY_CREATION_FEE_WALLET && !process.env.PROTOCOL_TREASURY_WALLET) issues.push({ code: "COMMUNITY_CREATION_FEE_WALLET_MISSING", severity: "fatal", message: "A treasury wallet is required to verify 1 SOL community creation access." });
    if ((process.env.ENABLE_LOCAL_STAKING_ACCOUNTING ?? "false") === "true") issues.push({ code: "LOCAL_STAKING_IN_PRODUCTION", severity: "fatal", message: "ENABLE_LOCAL_STAKING_ACCOUNTING must be false in production." });
    if ((process.env.ENABLE_PRODUCTION_STAKING ?? "false") === "true") issues.push({ code: "PRODUCTION_STAKING_UNSUPPORTED", severity: "fatal", message: "Production staking cannot be enabled until audited custody/freeze routes are implemented." });
    if (((process.env.ENABLE_AI_IMAGE_GENERATION ?? "false") === "true" || (process.env.ENABLE_STUDIO_IMAGE_GENERATION ?? "false") === "true") && (process.env.PAID_AI_GENERATION_ENABLED ?? "false") !== "true") issues.push({ code: "PAID_AI_AUTO_ENABLED", severity: "fatal", message: "Paid AI generation cannot be enabled in production without PAID_AI_GENERATION_ENABLED=true and explicit user-triggered cost guard." });
